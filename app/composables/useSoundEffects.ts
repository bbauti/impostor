...
const SOUND_CONFIG: Record<SoundType, SoundConfig> = {
  ...
  countdownTick: { src: '/sounds/countdown-tick.ogg', volume: 0.4, preload: true }
};

export const useSoundEffects = () => {
  const isMuted = useState<boolean>('soundMuted', () => false);
  const audioCache = useState<Map<SoundType, HTMLAudioElement>>('audioCache', () => new Map());
  ...
  const play = (type: SoundType) => {
    if (!import.meta.client || isMuted.value) return;
