...
const confirmVote = () => {
  ...
  // Don't clear selectedTarget - let the server confirmation handle the UI update
}
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col">
      <ProseH2 class="my-0">Fase de votación</ProseH2>
      <ProseP class="my-0">Quien sera el impostor?</ProseP>
    </div>

    <!-- Processing Votes Loader -->
    <div v-if="allPlayersVoted" class="flex flex-col items-center justify-center py-12">
      <Icon name="lucide:loader-circle" class="animate-spin text-6xl mb-4" />
      <ProseH3 class="my-0">Procesando votos...</ProseH3>
      <ProseP class="text-dimmed mt-2">Calculando resultados</ProseP>
    </div>
    ...
    <template v-else>
      ...
      <div class="border border-default rounded-lg p-4 space-y-3">
        ...
        <!-- Players who voted -->
        <div v-if="playersWhoVoted.length > 0" class="space-y-2">
          <div
        ...
        <div v-if="playersWhoHaventVoted.length > 0" class="space-y-2">
          <div
      ...
      <!-- Voted Status -->
      <div v-if="hasVoted" class="p-6 bg-success/50 border border-success rounded-lg text-center">
        <div class="text-2xl mb-2">✅</div>
        <div class="font-semibold">Has votado!</div>
        <div class="text-sm mt-1">Esperando a los otros jugadores...</div>
      </div>

      <!-- Voting Interface -->
      <div v-else-if="!showConfirmation" class="space-y-4">
